...
namespace GA_Lib
{
    ...
    {
        int numRepetitiveRuns = 30;  //重複執行次數
        int city  = 29;
        double[,]   distance;
        double[,]   node;
        int         count = 0;
        ...
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < ga.numRepetitiveRuns; i++)
            {
                ...
                ga.init_bays29();
                ga.SetStrategy(GAOption.Select.Tournament, GAOption.Crossover.NonRepeatableInteger.PMX, GAOption.Mutation.NonRepeatableInteger.Two_Swap);
                ga.Init(100, 29, 0, 28, GAOption.EncodingType.Integer, GAOption.RepeatableOption.NonRepeatable);
            ...
            fileWriter.WriteLine("BS, SA, SSD");
        ...
        public void init_tsp()      //座標讀檔
        {
            ...
            StreamReader str1 = new StreamReader("bays29.txt");
        ...
        {
            StreamReader str1 = new StreamReader("tsp_29.txt");
            string all = str1.ReadToEnd();
            string[] data = all.Split(' ', '\n', ',');
            int count = 0;
            distance = new double[city, city];
